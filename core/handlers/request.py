...
def handle_request(request):
    ...
    logger.info(
        f"REQUEST | ID={request.request_id} | PATH={request.path} | METHOD={request.method} | USER={getattr(request.user, 'email', 'anon')}"
    )
    ...
    if getattr(settings, "REQUIRE_API_KEY", False):
        ...
        if not api_key or api_key != getattr(settings, "API_KEY_VALUE", None):
            logger.warning(f"REQUEST BLOCKED | ID={request.request_id} | Motivo=API Key missing/invalid")
    ...
    if getattr(settings, "REQUIRE_TENANT_HEADER", False):
        ...
        if not tenant_id:
            logger.warning(f"REQUEST BLOCKED | ID={request.request_id} | Motivo=Tenant ID missing")
